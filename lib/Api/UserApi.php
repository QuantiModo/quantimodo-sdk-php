...
class UserApi
{
     ...
     *
     * @throws \QuantiModo\Client\ApiException on non-2xx response
     * @return \QuantiModo\Client\QuantiModo\Client\Model\User
     */
     ...
     *
     * @throws \QuantiModo\Client\ApiException on non-2xx response
     * @return array of \QuantiModo\Client\QuantiModo\Client\Model\User, HTTP status code, HTTP response headers (array of strings)
     */
    public function v1UserMeGetWithHttpInfo()
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\QuantiModo\Client\QuantiModo\Client\Model\User',
                '/v1/user/me'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\QuantiModo\Client\QuantiModo\Client\Model\User', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\QuantiModo\Client\QuantiModo\Client\Model\User', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
            }
